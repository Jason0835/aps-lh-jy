...
执行记录：编译成功；隔离相关测试 33 个、备注新增定向测试 13 个通过。完整 `NewSpecProductionStrategyRegressionTest` 存在既有测试隔离问题，本次运行结果为 64 个失败、3 个错误，失败集中在无关新增排产场景；单独复跑首日顺延用例确认准入结果为 false，失败原因为既有首检数量落入前一中班。
