...
`prepareShortageQuota(...)` 在“窗口日计划为 0、月底也无后续计划且存在历史欠产”时设置 `forceEndingByNoFuturePlan=true`。续作策略随后把 `isEnding` 设为 true，并把严格目标量设置为硫化余量 83，但没有把 SKU 的 `skuTag` 同步为 `SkuTagEnum.ENDING`。
...
3. 保留当前硫化余量严格目标量、窗口目标量和剩余额度同步逻辑；
4. 扩充现有中文日志，记录原标签、更新后标签、收尾剩余天数和严格目标量。
