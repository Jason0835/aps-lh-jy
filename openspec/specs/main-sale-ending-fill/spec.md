...
系统 SHALL 使用硫化参数 `SYS0304029` 控制主销/常规 SKU 收尾补满是否生效。参数为 `1` 时允许按本规则自动补量，参数为 `0` 时不得执行额外补量。

#### Scenario: 参数开启

- **WHEN** `SYS0304029 = 1`
- **AND** SKU 满足本规则全部收尾补满条件
- **THEN** 系统 SHALL 继续补满当天中班与下一个夜班计划量

#### Scenario: 参数关闭

- **WHEN** `SYS0304029 = 0`
- **AND** SKU 满足产品类型、运行态共用胎胚、胎胚在机和 20:00 后收尾等条件
- **THEN** 系统 SHALL NOT 因本规则补满当天中班或下一个夜班
- **AND** 系统 SHALL NOT 登记收尾补满允许超目标量
- **AND** 系统 SHALL 按 SKU 原目标量、硫化余量及普通排产逻辑完成收尾
- **AND** 系统 SHALL NOT 影响奇偶修正、续作降模补满夜班或其他已有补量规则
...
- **AND** 系统 SHALL 将本次补满新增量登记为收尾补满允许超目标量
- **AND** 后续严格收尾目标复核、SKU实际消费账本裁剪和结果校验 SHALL 保留该允许超目标量，不得将补满夜班回裁为普通超排
