...
系统 SHALL 复用现有 `TargetScheduleQtyResolver` 及相关工具计算清尾目标量。非共用胎胚 SKU 的基础清尾目标量为 `max(硫化余量, 胎胚库存)`；共用胎胚 SKU 的基础清尾目标量为 `硫化余量`，不得使用胎胚库存抬高共用胎胚 SKU 的清尾目标量。普通收尾目标量还 MUST 继续复用模台数归整、奇数余量调整、主销产品收尾等既有规则链路；命中胎胚库存硬目标时属于精确目标例外，优先使用已完成单胎胚库存赋值或共用胎胚分摊后的 SKU 硬目标，不得再次按模台数归整。
...
#### Scenario: 胎胚库存精确硬目标优先于模台数归整

- **Given** SKU 命中胎胚库存硬目标
- **And** 精确硬目标为 31
- **And** 模台数为 2
- **When** 系统执行当前窗口收尾判断或排后最终收尾判断
- **Then** 清尾比较目标量 MUST 为 31，不得向上归整为 32
- **AND** 本次窗口实际总排产量达到 31 时，`finalTailFlag` MUST 为 true
- **AND** 同一 SKU 的全部排程结果 `isEnd` MUST 统一为 `1`
