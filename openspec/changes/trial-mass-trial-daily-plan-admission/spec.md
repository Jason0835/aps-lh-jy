...
规范试制、量试 SKU 在续作和新增排产前的日计划准入判断，确保系统按排程窗口和可提前生产完整范围读取对应产品状态的月计划日计划量。整个范围无正计划量时直接进入未排，避免续作或后置入口继续消耗机台、胎胚和排产额度。
...
系统 SHALL 在完成 MES 和滚动续作识别后、进入续作排产策略前，通过 `PendingSkuUnscheduledRule.evaluateContinuousTrialDailyPlanAdmission` 对施工阶段为 `01/02` 的续作 SKU 执行完整范围日计划准入。

续作判断 SHALL 只使用月计划原始 `DAY_N`，不得使用历史欠产、硫化余量、胎胚库存或扣减后额度放行。前日排程 T+1 交替计划只属于非续作准入条件，不得放行完整范围全零的续作试制、量试 SKU。

#### Scenario: 试制或量试 SKU 已识别为续作

- **WHEN** 试制或量试 SKU 已按物料和产品状态匹配到 MES 或滚动续作机台
- **AND** `scheduleDate` 至 `windowEndDate + earlyProductionDaysThreshold` 的月计划日计划量全部小于等于 0
- **THEN** 系统 SHALL 将该 SKU 从全部续作机台副本、结构待排集合、活跃胎胚集合和后置索引移除
- **AND** 系统 SHALL 写入唯一未排原因“试制、量试月计划排产量全部为0，跳过排产”
- **AND** 未排数量 SHALL 为 0

#### Scenario: 续作试制量试范围内存在日计划

- **WHEN** 已识别为续作的试制或量试 SKU 在完整判断范围任一天的日计划量大于 0
